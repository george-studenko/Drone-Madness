...
class Tello:
    ...
    def __init__(self):
        ...
        self.droneSocket = socket.socket(socket.AF_INET,  # Internet
                                         socket.SOCK_DGRAM)  # UDP
        #self.droneSocket.bind((self.IP_DRONE, self.DRONE_UDP_PORT))  # For UDP response (receiving data)
        self.response = None
        self._ = None
    ...
    def run_udp_receiver(self):
        ...
        while True:
            try:
                #self.droneSocket.sendto('command'.encode('latin-1'), self.droneAddress)
                self.response, _ = self.droneSocket.recvfrom(1024)  # buffer size is 1024 bytes
            except Exception as e:
                print(e)
                break

    def run_state_receiver(self):
        """Setup drone UDP receiver. This method listens for responses of Tello. Must be run from a background thread
        in order to not block the main thread."""
    ...
    def collect_state(self, state):
        dic = {k: v for k, v in zip(self.STATE, ['' for _ in self.STATE])}
        items = state.split(';')
    ...
    def get_udp_video_address(self):
        return 'udp://@' + self.VIDEO_STREAM_UDP_IP + ':' + str(self.VIDEO_STREAM_UDP_PORT)  # + '?overrun_nonfatal=1&fifo_size=5000'
